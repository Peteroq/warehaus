...
const HOME_SECTIONS: SectionEntry[] = [
  ...
  },
  {
    ...
    description:
      'Dream, Design, Develop \u2014 each realm has its own mentor and familiar, working together to bring your vision to life.',
    tag: '\u4E09\u3064\u306E\u67F1',
    code: '0512',
  },
  {
    key: 'codex',
    heading: 'The collected works.',
    description:
      'Artifacts forged, quests undertaken, fragments explored. Every creation documented in the Codex.',
    tag: '\u3082\u306E\u3065\u304F\u308A',
    code: '0618',
  },
  {
    key: 'familiars',
    heading: 'Bound familiars, guided by masters.',
    description:
  ...
  },
  {
    ...
    description:
      'Every great creation starts with a conversation. Tell us what you\'re building.',
    tag: '\u59CB\u3081\u307E\u3057\u3087\u3046',
    code: '0899',
  },
];
...
function buildCodexEntrySections(slug: string): SectionEntry[] {
  ...
  if (entry.results && entry.results.length > 0) {
    sections.push({
      ...
      tag: '\u7D50\u679C',
      code: '0905',
      details: entry.results.map((r, i) => ({ label: `Result`, value: r })),
    });
  }
...
function getSectionsForPath(pathname: string): SectionEntry[] {
...
function SectionBlock({ data, isOnLight }: { data: SectionEntry; isOnLight: boolean }) {
  return (
    <>
      <h2
...
export function RightSidebar() {
  ...
  const isCodexPage = pathname === '/codex';
    ...
    >
      {rightCollapsed ? (
        ...
        <div className="flex flex-col h-full p-6">
          ...
          {/* Scroll-synced / hover-synced content */}
          <div ref={containerRef} className="flex-1 overflow-hidden relative">
            {isCodexPage ? (
              /* Codex: fade between sections based on activeSection (hover) */
              <div className="relative h-full">
                {SECTION_CONTENT.map((data) => {
                  const isActive = data.key === activeSection;
                  return (
                    <div
                      key={data.key}
                      className={`absolute inset-0 flex flex-col py-4 transition-all duration-400 ${
              ...
              /* All other pages: scroll-synced translateY */
              <div
                ref={contentRef}
